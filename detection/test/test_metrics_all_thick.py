...
# 0. 모든 선 1px 두껍게 처리 함수 
def thicken_lines_uniform(image_np):
    """
    모든 어두운 선을 통일 1px 두껍게 처리
    """
    gray = cv2.cvtColor(image_np, cv2.COLOR_RGB2GRAY)
    dark_mask = (gray < 50).astype(np.uint8) * 255  # 어두운 픽셀
    
    # 모든 선 1px 두껍게!
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    thickened = cv2.dilate(dark_mask, kernel, iterations=1)
    thickened_mask = (thickened > 0).astype(np.uint8)
    
    # 어두운 선 강조 (RGB 모두 20으로)
    image_np[thickened_mask == 1] = np.minimum(image_np[thickened_mask == 1], [20, 20, 20])
    return image_np
...
checkpoint = torch.load('../checkpoint/clean.pth', map_location=device)
...
def evaluate_one(image_path, json_path, iou_thr=0.5, dilate=True):
    ...
    # 모든 선 1px 두껍게 처리
    print(f"🔍 Applying uniform 1px line thickening to {os.path.basename(image_path)}")
    image_np = thicken_lines_uniform(image_np)
...
# tqdm 추가된 루프
for img_path in tqdm(image_paths, desc="Evaluating images (Uniform 1px thickening)"):
...
print("\n=== Dataset metrics (mean over images) - Uniform 1px thickening applied ===")
...
with open('./result/gt_clean_uniform_1px.json', 'w', encoding='utf-8') as f:
    json.dump(save_dict, f, indent=2, ensure_ascii=False)

print(f"\n결과 저장 완료: ./result/gt_clean_uniform_1px.json")